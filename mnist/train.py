...
def main():
    ...
    parser.add_argument('-s', '--seed', type=int, metavar='S',
                        help='random seed')
    parser.add_argument('--log-interval', type=int, default=10, metavar='N',
                        help='how many batches to wait before logging training status')
    parser.add_argument('--dataset', type=str, default='mnist',
                        choices=['mnist', "fmnist"], help='dataset to use')
    parser.add_argument('--save-model', action='store_true', default=False,
                        help='For Saving the current Model')
    parser.add_argument('--arch', type=str, required=True)
    parser.add_argument('--optimizer', type=str, default="Adam")
    parser.add_argument('--init', type=str, default="", choices=["", "xavier", "he"])
    parser.add_argument('-nt', '--network_type', type=str, choices=["lrelu", "rn", "rrn"])
    ...
    global writer
    global cnt
    activation_function_key = args.network_type
    print("---" * 25)
    ...
    scheduler = None
    if "rn" in activation_function_key:
        if args.optimizer.lower() == "adam":
    ...
    scores_df.to_csv(f"scores_sl/{args.arch}_scores_{args.dataset}/scores_{args.arch}_{activation_function_key}_{args.seed}.csv")
    if args.save_model:
